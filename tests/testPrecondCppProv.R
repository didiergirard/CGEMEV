...
# gd=grid.domain
print(system.time(ex1With256.gd <- grid.domain(ex1.md,ngrid<-256)))
#
#
...
z <- gm$look[1:gm$n1,1:gm$n1][! ex1With256.gd$missing.sites]
cat("number of observations\n",length(z),"\n")
plotUncompleteGrid.gaussian.matern(gm, ex1With256.gd)
#
sum(z**2)/ex1With256.gd$non.missing.number
cat("-> done\n")
...
system.time(print(out <-
	 fsai11Precond.GEevalOnThetaGrid(z, candidateThetas.Grid, gm$smoothness, ex1With256.gd ,tolPGC=1e-03)))

plot( candidateThetas.Grid, out$values, type="l")

system.time(print(out <-
	 fsai11Precond.GEbisectionLogScaleSearch(z,
		  gm$smoothness, ex1With256.gd ,tolPGC=1e-03, 1, 100,  1e-04)))


system.time(print(out <-
	 fsai11ThreePreconds.GEbisectionLogScaleSearchNEW(z,
		  gm$smoothness, ex1With256.gd ,tolPGC=1e-03, 1, 100,  1e-04)))
